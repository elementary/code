    ...
    public class MainWindow : Hdy.Window {
        ...
        private void init_layout () {
            ...
            folder_manager_view.activate.connect ((a) => {
                ...
                //TODO Foldermanager should only show text files so is this check required
                if (file.is_valid_textfile) {
                    document_view.open_document.begin (a);
                } else {
                   ...
                   if (uri != "") {
                        ...
                        */
                        if (file.query_exists ()) {
                            ...
                            // We already know the file exists and a restore override path is not going to match a temporary file
                            if (restore_override != null && (file.get_path () == restore_override.file.get_path ())) {
                                yield document_view.open_document (uri, true, -2, restore_override.range);
                                was_restore_overriden = true;
                            } else {
                                yield document_view.open_document (uri, was_restore_overriden ? false : is_focused, pos);
                            }
                        }
                    }
                }
